...
const NUMBER_ARGUMENT = 'number-integer';
...
function saveToDb(numberToSave) {
    const options = {
        url: 'https://robo-gallerist.firebaseio.com/picture.json',
        method: 'PUT',
        headers: {
...
function processV1Request (request, response) {
  ...
  const actionHandlers = {
    ...
    'showRandomArt': () => {
      let clientName = app.getArgument(GIVEN_NAME_ARGUMENT);
      var randomNumber = Math.floor(Math.random() * 11)+1;
      if (requestSource === googleAssistantRequest) {
        saveToDb(randomNumber);
        sendGoogleResponse('Hello ' + clientName + ', I found this Art for you '); // Send simple response to user
      } else {
        sendGoogleResponse('Hello ' + clientName + ', I found this Art for you '); // Send simple response to user
      }
    },
    ...
    'showMoreArt': () => {
      ...
      if (requestSource === googleAssistantRequest) {
        saveToDb(randomNumber);
        sendGoogleResponse('How about this one '); // Send simple response to user
      } else {
        sendGoogleResponse('How about this one '); // Send simple response to user
      }
    },
